...
u0 = ones(m^2,1)
[uvec,fl2,rr2,it2,rv2] = pcg(A_sym,F,[], 80,[],[],u0);

uvec=-1*uvec    %convert result from utilde back to u for original A
...
u_exact = A\F;
err=u_exact - uvec
err_norm =max(abs(uvec-u_exact))
u0 = -1*uvec
[uvec2,fl3,rr3,it3,rv3] = pcg(A_sym,F,[],10,[],[],u0);
uvec2 = -1*uvec2
err_norm2 = max(abs(uvec2-u_exact))
usoln(Iint,Jint) = reshape(uvec,m,m);

% assuming true solution is known and stored in utrue:

infn= norm(usoln-utrue,inf)
err2 = max(max(abs(usoln-utrue)));   
