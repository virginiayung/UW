...
switch method
   ...
   case 'GS'
      ...
      N = UA;
   case 'SOR'
      omega_vals = [ 2 / (1 + sin(pi*h))]%[0.01:0.01:2];
    ...
    ntest = length(omega_vals);

    for otest=1:ntest
        omega = omega_vals(otest)
...
% compute spectral radius of iteration matrix G:
    G= M\N;
    rhoG(otest) = max(abs(eig(full(G))));
    disp(sprintf('Spectral radius of G for %s with omega=%5f is %5f',method,omega_vals(otest),rhoG(otest)))

 figure(3)
    plot(omega_vals(otest),rhoG(otest), 'o-')
   hold on
...
%legend('Jacobi','GS','SOR', 'SOR-1.8','SOR-1.95','SOR-1.99')
